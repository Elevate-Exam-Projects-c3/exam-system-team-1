...
{
    public static async Task SeedAsync(AppDbContext context, ILogger logger,RoleManager
        <IdentityRole> _roleManger,UserManager<AppUser> userManger)
    {
        try
        {
            ...
            //1. Seed Two Roles (Student,Instrctor)
            var flag=  await _roleManger.RoleExistsAsync("Student");
            if (!flag)
            {
                await _roleManger.CreateAsync(new IdentityRole("Student"));
                    
            }
            var secondFlag = await _roleManger.RoleExistsAsync("Instructor");
            if (!secondFlag)
            {
               await _roleManger.CreateAsync(new IdentityRole("Instructor"));
            }
            var registerDto = new RegisterDto()
            {
                Email = "ahmed.backend33@gmail.com",
                FullName = "Main Instrctor",
                Password = "Password@123456"
            };
            var user= await userManger.FindByEmailAsync(registerDto.Email);
            if(user == null)
            {
               ...
               var result = await userManger.CreateAsync(appUser, registerDto.Password);

                if (result.Succeeded)
                {
                    await userManger.AddToRoleAsync(appUser, "Instructor");
